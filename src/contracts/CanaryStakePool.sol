...
contract CanaryStakePool is ICanaryStakePool, Pausable, Owned {
    ...
    mapping(address token => mapping(BondType => address)) public stakeTokenAddressMap;
    ...
    constructor(address admin, address[] memory initialTokensWhitelist) Owned(admin) {
        for (uint256 i; i < initialTokensWhitelist.length; i++) {
            tokenWhitelist[initialTokensWhitelist[i]] = true;
        }
        canaryClaimNFT = new CanaryStakeClaimNFT("Canary Stake NFT", "C-STAKE-NFT", address(this));
    }

    function deposit(address token, uint256 amount, BondType bondType) external whenNotPaused returns (uint256) {
        ...
        stakeToken.underlyingToken.safeTransferFrom(msg.sender, address(this), amount);

        uint256 shares = stakeToken.stakingToken.deposit(amount, msg.sender);

        emit Deposit(token, stakeTokenAddress, msg.sender, bondType, amount, shares);

        return shares;
    }

    function requestWithdraw(address stakeTokenAddress, uint256 shares) external whenNotPaused returns (uint256) {
        StakeBondToken memory stakeToken = stakeBondToken[stakeTokenAddress];
        ...
        uint256 tokenId =
            canaryClaimNFT.safeMint(msg.sender, address(stakeToken.underlyingToken), claimAmount, stakeToken.bondType);

        emit WithdrawalRequest(
            underlyingToken, stakeTokenAddress, msg.sender, stakeToken.bondType, tokenId, claimAmount, shares
        );
    ...
    function claim(uint256 tokenId) external whenNotPaused returns (uint256) {
        ...
        if (bondType == BondType.OneWeek && (block.timestamp - creationTimestamp) < 1 weeks) {
            revert BondNotMature();
        } else if (bondType == BondType.FourWeeks && (block.timestamp - creationTimestamp) < 4 weeks) {
            revert BondNotMature();
        }
    ...
    function adminYieldDeposit(address stakeTokenAddress) external whenNotPaused onlyOwner {
        (uint256 yield, uint256 lastTimestamp) = calculatePendingYield(stakeTokenAddress);
        ...
        if (yield > 0) {
            ...
            stakeToken.underlyingToken.safeTransferFrom(msg.sender, address(stakeToken.stakingToken), yield);
    ...
    function calculatePendingYield(address stakeTokenAddress)
        public
        view
        returns (uint256 yield, uint256 lastTimestamp)
    {
        StakeBondToken memory stakeToken = stakeBondToken[stakeTokenAddress];
        ...
        yield = (totalSupply * APY_BPS * timeElapsed) / (10000 * SECONDS_IN_A_YEAR);

        return (yield, lastTimestamp);
    }

    function _createBondToken(address token, BondType bondType) internal returns (address) {
        ...
        tokenName = string.concat("Canary st", ERC20(token).symbol(), " ", bondTypeString, " notice");
        tokenSymbol = string.concat("CANARY-ST", ERC20(token).symbol(), "-", bondTypeString);

        IERC4626 canaryStakeToken = IERC4626(
            address(new CanaryStakeToken(token, tokenName, tokenSymbol, ERC20(token).decimals(), address(this)))
        );
        ERC20(token).approve(address(canaryStakeToken), type(uint256).max);

        StakeBondToken storage stakeBondTokenStg = stakeBondToken[address(canaryStakeToken)];
        ...
        emit StakeTokenDeployed(address(stakeBondTokenStg.stakingToken), token, bondType);
